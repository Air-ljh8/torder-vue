...
const { setUserSelectIngredientIds } = store;

const handleIngredientClick = (id: number) => {
  if (userSelect.value.ingredientIds.includes(id)) {
    setUserSelectIngredientIds(
      userSelect.value.ingredientIds.filter(
        (ingredientId) => ingredientId !== id
      )
    );
    return;
  }
  setUserSelectIngredientIds([...userSelect.value.ingredientIds, id]);
};

const getUserSelectStyle = (ingredient: Ingredient) => ({
  'flex h-fit w-fit items-center justify-center rounded-full border-[3px]':
    true,
  'border-secondary-light': userSelect.value.ingredientIds.includes(
    ingredient.id
  ),
  'border-gray_00-light dark:border-gray_00-dark':
    !userSelect.value.ingredientIds.includes(ingredient.id),
});
</script>
